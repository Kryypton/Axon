...
import com.radonn.axon.models.wow.models.guild.GuildRoster;
import com.radonn.axon.models.wow.models.mythicKeystoneProfile.MythicKeystoneProfile;
import com.radonn.axon.models.wow.models.mythicKeystoneProfile.MythicKeystoneProfileSeason;
...
public class BlizzardApiController {
    ...
    public Character getCharacter(@PathVariable String realm, @PathVariable String name) throws CharacterNotFoundException {
        try {
            return fetchData(realm.toLowerCase(), name.toLowerCase(), "/profile/wow/character/{realm}/{name}", Character.class);
        } catch (FetchDataException e) {
            throw new CharacterNotFoundException("Personnage non trouvé dans l'API.");
        }
    }

    @GetMapping("/{realm}/{name}/equipment")
    public Equipment getEquipment(@PathVariable String realm, @PathVariable String name) throws EquipmentNotFoundException {
        try {
    ...
    @GetMapping("/{id}")
    public PlayableSpecialization getPlayableSpecialisation(@PathVariable int id) throws PlayableSpecializationNotFoundException {
        try {
    ...
    public Achievements getAchievements(@PathVariable String realm, @PathVariable String name) throws AchievementsNotFoundException{
        try {
    ...
    public MythicKeystoneProfileSeason getMythicKeystoneProfileSeason(@PathVariable String realm, @PathVariable String name, @PathVariable int seasonIdIndex) throws MythicKeystoneProfileSeasonNotFoundException{
        try {
    ...
    public CharacterMedia getCharacterMedia(@PathVariable String realm, @PathVariable String name) throws CharacterMediaNotFoundException{
        try {
	...
	public MythicKeystoneProfile getMythicKeystoneProfile(@PathVariable String realm, @PathVariable String name) throws MythicKeystoneProfileNotFoundException {
        try { 
            return fetchData(realm.toLowerCase(), name.toLowerCase(), "/profile/wow/character/{realm}/{name}/mythic-keystone-profile", MythicKeystoneProfile.class);
        } catch (FetchDataException e) {
    ...
    public GuildRoster getGuildRoster(@PathVariable String realm, @PathVariable String guild_name) throws GuildRosterNotFoundException {
        try {
    ...
    private <T> T fetchData(String realm, String name, int id, String path, Class<T> responseType) throws FetchDataException {
        try {
            return webClient.get()
                .uri(uriBuilder -> uriBuilder
                    .path(path)
