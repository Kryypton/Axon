        ...
        public static void lgeMenu(ButtonInteractionEvent event) {
                switch (event.getComponentId()) {
                        ...
                        case "lge_menu_entretien":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        AxonApplication.UserLgeCtrl
                                                        .deleteCharacterByDiscordID(event.getUser().getIdLong());
                        ...
                        case "lge_menu_entretien_charte":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        FileUploadBuilder charte = FileUploadBuilder.fromPath("charte.png",
                                                        "src/main/resources/images/guild/icons/charte.png");
                                        event.editMessageEmbeds(
                                                        Embeds.lgeMenuEntretienCharte(charte.getOutPutName()).build())
                                                        .setAttachments(charte.getFileUpload())
                                                        .setActionRow(ItemComponents.lgeMenuEntretienCharte(event))
                                                        .queue();
                                });
                                break;
                        ...
                        case "lge_menu_entretien_rules":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        FileUploadBuilder rules = FileUploadBuilder.fromPath("rules.png",
                                                        "src/main/resources/images/guild/icons/rules.png");
                                        event.editMessageEmbeds(
                                                        Embeds.lgeMenuEntretienRules(rules.getOutPutName()).build())
                                                        .setAttachments(rules.getFileUpload())
                                                        .setActionRow(ItemComponents.lgeMenuEntretienRules(event))
                                                        .queue();
                                });
                                break;
                        ...
                        case "lge_menu_entretien_cgu":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        try {
                                                FileUploadBuilder cgu = FileUploadBuilder.fromPath("cgu.png",
                                                                "src/main/resources/images/guild/icons/cgu.png");
                                                event.editMessageEmbeds(
                                                                Embeds.lgeMenuEntretienCGU(cgu.getOutPutName()).build())
                                                                .setAttachments(cgu.getFileUpload())
                                                                .setActionRow(ItemComponents.lgeMenuEntretienCGU(event))
                                                                .queue();
                                        } catch (Exception e) {
                                                e.printStackTrace();
                                        }
                        ...
                        case "lge_menu_entretien_global_finish":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        FileUploadBuilder cgu = FileUploadBuilder.fromPath("valid.png",
                                                        "src/main/resources/images/guild/icons/valid.png");
                                        event.editMessageEmbeds(
                                                        Embeds.lgeMenuEntretienFinish(cgu.getOutPutName()).build())
                                                        .setAttachments(cgu.getFileUpload())
                                                        .setActionRow(ItemComponents.lgeMenuEntretienFinish(event))
                        ...
                        case "lge_menu_entretien_complete":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        try {
                                                ...
                                                event.editMessageEmbeds().setEmbeds(Embeds.loading().build())
                                                ...
                                                Character character = AxonApplication.BnetCtrl.getCharacter("hyjal",
                                                                user.getName().toLowerCase());
                                                // event.getMember().modifyNickname(character.getName()).queue();
                                                ArrayList<Role> roles = new ArrayList<Role>();
                                                // roles.add(event.getGuild().getRolesByName("Novice", true).size() != 0
                                                // ?
                                                // event.getGuild().getRolesByName("Novice", true).get(0) :
                                                // event.getGuild().createRole().setName("Novice").complete().getJDA().getRolesByName("Novice",
                                                // true).get(0));
                                                // roles.add(event.getGuild().getRolesByName("---- Bloc Principal ----",
                                                // true).size() != 0 ? event.getGuild().getRolesByName("---- Bloc
                                                // Principal
                                                // ----", true).get(0) : event.getGuild().createRole().setName("----
                                                // Bloc
                                                // Principal ----").complete().getJDA().getRolesByName("---- Bloc
                                                // Principal
                                                // ----", true).get(0));
                                                roles.add(event.getGuild()
                                                                .getRolesByName(character.getRace().getName(), true)
                                                                .size() != 0
                                                                                ...
                                                                                : event.getGuild().createRole().setName(
                                                                                                ...
                                                                                                .get(0));
                                                roles.add(event.getGuild()
                                                                .getRolesByName(character.getCharacterClass().getName(),
                                                                                true)
                                                                .size() != 0
                                                                                ? event.getGuild().getRolesByName(
                                                ...
                                                roles.add(event.getGuild()
                                                                .getRolesByName(character
                                                                                .getActiveSpec().getName(), true)
                                                                .size() != 0
                                                                                ? event.getGuild().getRolesByName(
                                                                                                character.getActiveSpec()
                                                                                                                .getName(),
                                                                                                true).get(0)
                                                                                : event.getGuild().createRole().setName(
                                                                                                character.getActiveSpec()
                                                                                                ...
                                                                                                .get(0));
                                                roles.add(event.getGuild()
                                                                .getRolesByName(character.getPlayableSpecialization()
                                                                                .getRole().getName(), true)
                                                                .size() != 0 ? event.getGuild()
                                                                                .getRolesByName(character
                                                                                ...
                                                                                : event.getGuild().createRole()
                                                                                                ...
                                                                                                .complete().getJDA()
                                                                                                .getRolesByName(
                                                                                                                character.getPlayableSpecialization()
                                                                                                                                .getRole()
                                                                                                                                .getName(),
                                                                                                                true)
                                                                                                .get(0));
                                                roles.add(event.getGuild()
                                                                .getRolesByName(character.getFaction().getName(), true)
                                                                .size() != 0
                                                                                ? event.getGuild().getRolesByName(
                                                ...
                                                event.getGuild().removeRoleFromMember(event.getMember(),
                                                                event.getGuild().getRolesByName("0 - arrivé", true)
                                                                                .get(0))
                                                                .queue();
                                                event.getGuild().getTextChannelById("1057477730830143589")
                        ...
                        case "lge_menu_entretien_complete_integration":
                                event.getJDA().getGatewayPool().submit((Runnable) () -> {
                                        try {
                                                ...
                                                GuildCharacter user = AxonApplication.UserLgeCtrl
                                                                .getCharacterMainByDiscordId(event.getIdLong());
                                                Character character = AxonApplication.BnetCtrl.getCharacter("hyjal",
                                                                user.getName().toLowerCase());
                                                event.getMember().modifyNickname(character.getName()).queue();
                                                ArrayList<Role> roles = new ArrayList<Role>();
